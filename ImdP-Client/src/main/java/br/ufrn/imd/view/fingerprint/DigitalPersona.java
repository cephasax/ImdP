...
public class DigitalPersona {
	...
	public byte[] principal() {
		...
		listReaders();
		DPFPTemplate temp = getTemplate(null, 1);
		b = temp.serialize();
		return b;
	}

	public boolean verificar(byte[] digital) {
		b = digital;
	...
	public void insert(byte[] digital) {
	...
	public DPFPTemplate getTemplate(String activeReader, int nFinger) {
		...
		try {
			...
			alert.setContentText("Escanei seu dedo " + enrollment.getFeaturesNeeded() + " vezes.");
			alert.show();
			...
			alert.setTitle("Feedback");
			alert.setHeaderText(null);
			alert.setContentText("Digital armazenada.");
			alert.show();
